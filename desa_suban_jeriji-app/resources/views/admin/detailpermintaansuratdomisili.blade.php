...
@section('title')
    Detail Permintaan Surat Keterangan Domisili
@endsection

@section('konten')


<h3 style="text-align: center">Data Permintaan Surat Keterangan Domisili:</h3>
<div class="container">

    <div class="row">
        <div class="col-lg-3">
            <img src="/ktp/{{ $domisili->profile->ktp->ktp }}" class="img-thumbnail" alt="..."><br>
        </div>
        <div class="col-lg-9">
            <table class="table table-bordered">
                <tr>
                    <th width="200">Nama</th>
                    <td>{{ $domisili->profile->nama }}</td>
                </tr>
                <tr>
                    <th width="200">NIK</th>
                    <td>{{ $domisili->profile->nik }}</td>
                </tr>
                <tr>
                    <th width="200">Tempat, Tanggal Lahir</th>
                    <td>{{ $domisili->profile->tempat }},  {{ $domisili->profile->ttl }}</td>
                </tr>
                <tr>
                    <th width="200">Jenis Kelamin</th>
                    <td>{{ $domisili->profile->gender }}</td>
                </tr>
                <tr>
                    <th width="200">Warganegara/Agama</th>
                    <td>{{ $domisili->profile->warga }}/{{ $domisili->profile->agama }}</td>
                </tr>
                <tr>
                    <th width="200">Status Menikah</th>
                    <td>{{ $domisili->profile->nikah }}</td>
                </tr>
                <tr>
                    <th width="200">Pekerjaan</th>
                    <td>{{ $domisili->profile->pekerjaan }}</td>
                </tr>
                <tr>
                    <th width="200">Alamat</th>
                    <td>{{ $domisili->profile->alamat }}</td>
                </tr>
              </table>
      ...
      @if ($domisili->status === 'pengecekan')
          

      <form action="/permintaansuratdomisiliadmin/{{ $domisili->id }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('put')
        ...
        <div class="form-group col-xl-6 col-lg-8" style="display: none">
            ...
            <input type="text" class="form-control" name="nomor" id="nomor"  value="202/00{{ ($max->no+1) }}/SBJ/XII/2023">
